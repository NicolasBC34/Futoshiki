            ...
            for b in range(difficulte):
                if (((a != x) or (b != y)) and (grille_base[a][b] < 9)): #Vérifie que la case est ni selectionnée
                    ...
                    pygame.draw.rect(screen, color, pygame.Rect(b * (cot + mar) + 390, a * (cot + mar) +
                                                                200, cot, cot)) #Remet les cases en blanc
...
grille_rectangles = [[0] * 4 for i in
                     range(4)]  # grille interface (correspond aux rectangles qui sont des zones cliquables)

grille_finie = [[2, 1, 3, 4], [3, 4, 2, 1], [1, 2, 4, 3],
                [4, 3, 1, 2]]  # grille qu'on ira lire dans un fichier texte par la suite
...
for x in range(difficulte):
    for y in range(difficulte):
        rect = pygame.draw.rect(screen, color, pygame.Rect(x * (cot + mar) + 390, y * (cot + mar) + 200, cot,
